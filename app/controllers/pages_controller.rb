class PagesController < ApplicationController
  ...
  def cosmo
    ...
    @models = []
    maps_response = RestClient.post 'https://dane.imgw.pl/datastore/getFilesList', {path: "/Oper/COSMO/post_proc_#{params[:cosmo]}/maps_test/d#{path_time}", productType: 'oper'}
    maps_html = Nokogiri::HTML(maps_response.body)
    li_collection = maps_html.css('div').css('ul').css('li')
    li_size = li_collection.size()
    for i in 0..(li_size - 1)
      if li_collection[i].css('a')[0]['href'].include? ".00."
        @models << 'https://dane.imgw.pl/' + li_collection[i].css('a')[0]['href']
      end
    end

    graf_response = RestClient.post 'https://dane.imgw.pl/datastore/getFilesList', {path: "/Oper/COSMO/post_proc_#{params[:cosmo]}/graf/d#{path_time}", productType: 'oper'}
    graf_html = Nokogiri::HTML(graf_response.body)
    li_collection = graf_html.css('div').css('ul').css('li')
    li_size = li_collection.size()
    for i in 00..(li_size - 1)
      if li_collection[i].css('a')[0]['href'].include? ".00."
        @models << 'https://dane.imgw.pl/' + li_collection[i].css('a')[0]['href']
      end
    end
  ...
  def forecast
    ...
    @forecasts = @forecasts.to_json
  end
  
  class YrnoForecast
    def initialize(station_latitude, station_longitude, forecast_next, temperatures, wind_speeds, wind_directs, preasures, situations, precipitations, times_from, times_to, hour, date)
  ...
  def measurements
    ...
    csv.each do |row|
      station = Station.where(number: row['id_stacji']).last();
      @measurements << ImgwMeasurment.new(
        station.latitude,
        station.longitude,
        row['data_pomiaru'],
        row['godzina_pomiaru'],
  ...
  class ImgwMeasurment
    def initialize(station_latitude, station_longitude, date, hour, temperature, wind_speed, wind_direct, humidity, et, rainfall, preasure)
      @station_latitude = station_latitude
      @station_longitude = station_longitude
  ...
  def metar
    ...
    # for i in 0..10
      ...
      station = MetarStation.where(number: ogimet_metar[10..15]).first()
      hour = ogimet_metar[7..8]
      day = ogimet_metar[5..6]
      @metars << OgimetMetar.new(
        station.latitude.to_s,
        station.longitude.to_s,
        $cloud_cover,
        $wind_direct,
        $wind_speed,
        $visibility,
  ...
  class OgimetMetar
    def initialize(station_latitude, station_longitude, cloud_cover, wind_direct, wind_speed, visibility, 
        situation, pressure, ogimet_metar, message, hour, date)
      @station_latitude = station_latitude
      @station_longitude = station_longitude
