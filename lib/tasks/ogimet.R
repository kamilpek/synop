...
precipations <- function(code){
  ...
  message <<- paste(message, mes, sep=" ")
  visibility <<- paste(visibility, visibility_horizontal(substr(code, 4, 5)))
}
clouds <- function(code){
  ...
  message <<- paste(message, mes, sep=" ")
  cloud_cover <<- paste(cloud_cover, size(substr(code, 1, 1)))
  wind_direct <<- paste(wind_direct, direction(substr(code, 2, 3)))
  wind_speed <<- paste(wind_speed, substr(code, 4, 5))
}
first_group <- function(code){
  ...
  mes <- paste(mes, value)
  message <<- paste(message, mes, sep=" ")
  temperature <<- paste(temperature, value)
}
...
third_group <- function(code){
  ...
  if(substr(code, 2, 2) == 0){
    value <- sprintf("1%s.%s hPa", substr(code, 2, 4), substr(code, 5, 5))
  } else {
    value <- sprintf("0%s.%s hPa", substr(code, 2, 4), substr(code, 5, 5))
  }
  mes <- paste(mes, value)
  message <<- paste(message, mes, sep=" ")
  pressure <<- paste(pressure, value)
}
...
seventh_group <- function(code){
  ...
  message <<- paste(message, mes, sep=" ")
  situation <<- paste(situation, weather(substr(code, 2, 3)))
}
