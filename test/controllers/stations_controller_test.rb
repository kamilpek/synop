...
class StationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create station" do
    assert_difference('Station.count') do
      post stations_url, params: { station: { name: @station.name, number: @station.number, status: @station.status } }
    end
  ...
  test "should update station" do
    patch station_url(@station), params: { station: { name: @station.name, number: @station.number, status: @station.status } }
